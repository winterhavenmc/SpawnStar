...
public class PluginMainTests {
	...
	class PluginMainObjectTests {
		...
		@Test
		@DisplayName("spawn star utility not null.")
		void SpawnStarFactoryNotNull() {
			Assertions.assertNotNull(plugin.spawnStarUtility, "spawn star utility is null.");
		}
	}
	...
	class ConfigTests {
		...
		void GetLanguage() {
			Assertions.assertEquals("en-US", plugin.getConfig().getString("language"), "language does not equal 'en-US'");
		}
		...
		void ConfigFileKeyNotNull(String key) {
			...
			Assertions.assertTrue(enumConfigKeyStrings.contains(key), "file config key is not contained in ConfigSetting enum.");
		}
